...
export const fileStorage = pgTable("file_storage", {
  ...
  uploaderIdx: index("file_storage_uploader_idx").on(table.uploader_type, table.uploader_id),
  categoryIdx: index("file_storage_category_idx").on(table.file_category),
  statusIdx: index("file_storage_status_idx").on(table.status),
  hashIdx: index("file_storage_hash_idx").on(table.sha256),
}));

// 文件使用关联表 - 记录文件被哪些实体使用
export const fileUsage = pgTable("file_usage", {
  id: text('id').primaryKey(),
  file_id: text('file_id').notNull().references(() => fileStorage.id, { onDelete: 'cascade' }),
  ...
  fileIdx: index("file_usage_file_idx").on(table.file_id),
  entityIdx: index("file_usage_entity_idx").on(table.entity_type, table.entity_id),
}));
...
export const fileShare = pgTable("file_share", {
  ...
  tokenIdx: index("file_share_token_idx").on(table.share_token),
  fileIdx: index("file_share_file_idx").on(table.file_id),
}));
...
export const fileAccessLog = pgTable("file_access_log", {
  ...
  fileIdx: index("file_access_log_file_idx").on(table.file_id),
  userIdx: index("file_access_log_user_idx").on(table.user_id),
  dateIdx: index("file_access_log_date_idx").on(table.created_at),
  typeIdx: index("file_access_log_type_idx").on(table.access_type),
}));
...
export const fileTagRelation = pgTable("file_tag_relation", {
  id: text('id').primaryKey(),
  file_id: text('file_id').notNull().references(() => fileStorage.id, { onDelete: 'cascade' }),
  tag_id: text('tag_id').notNull().references(() => fileTag.id, { onDelete: 'cascade' }),
  created_at: timestamp('created_at').$defaultFn(() => new Date()).notNull(),
}, (table) => ({
  fileTagIdx: index("file_tag_relation_file_tag_idx").on(table.file_id, table.tag_id),
}));

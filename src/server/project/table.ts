...
import { uuidWithTimestamps } from "@/server/admin/db/columnsHelpers";
import { channel } from "../community/table";
...
export const projectComment = pgTable("project_comment", {
  ...
  parentId: uuid('parent_id').references(() => projectComment.id, { onDelete: 'cascade' }), // 回复评论
  ...
  index("project_comment_project_idx").on(table.projectId),
  index("project_comment_parent_idx").on(table.parentId),
]);

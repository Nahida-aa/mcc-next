...
import { notification, notificationReceiver, organization, projectMember, user } from "@/server/db/schema";
import { eq, and, lt, gte, ne, asc, desc, or, isNotNull, not, inArray, notInArray, is } from 'drizzle-orm';
import { AppError } from "../../openapi/middlewares/on-error";
...
export const getUserProjectPermissions = async (projectId: string, userId: string) => {
  ...
  if (member.length === 0) throw new AppError(404, "项目不存在");
...
export const inviteJoinProjectMember = async (
  ...
  if (memberExisting) throw new AppError(400, '用户已是项目成员');

  // 检查权限
  const hasPermission = await checkMemberPermission(projectId, inviterId, 'manage_invite');
  if (!hasPermission) throw new AppError(403, "权限不足：需要管理邀请权限");
...
export const acceptProjectMemberInvite = async (
  ...
  return await db.transaction(async (tx) => {
    ...
    const [updatedMember] = await tx
      ...
      .returning();
    if (!updatedMember) throw new AppError(404, '邀请已删除');
    // 2. 标记通知为已读
    await tx
...
export const updateProjectMember = async (
  ...
  if (!existingMember) throw new AppError(404, "成员不存在");
  // 检查权限
  const hasPermission = await checkMemberPermission(projectId, authId,'manage_member');
  if (!hasPermission||(existingMember.isOwner && (authId!==entityId))) throw new AppError(403, "权限不足");
  // 更新成员
...
export const removeProjectMember = async (
  ...
  // 检查权限
  const hasPermission = await checkMemberPermission(projectId, authId,'manage_member');
  if (!hasPermission) throw new AppError(403, "权限不足：需要成员管理权限");
  ...
  if (existingMember.length === 0) throw new AppError(404, "成员不存在");

  // 不能移除项目所有者
  if (existingMember[0].isOwner) throw new AppError(403, "不能移除项目所有者");

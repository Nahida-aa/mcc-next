import { Db, db } from '@/server/admin/db';
import { 
  community, 
  communityMember, 
  communityRole 
...
import { joinMethod, JoinMethodType, MemberJoinStats } from '../model/member';
...
export const addMember = async(db: Db, data: AddMemberParams) => {
  ...
  if (!validation.valid) throw AppErr(422, `加入数据验证失败: ${validation.errors.join(', ')}`);

  // 检查用户是否已经是成员
  const memberExisting = await checkMemberExisting(data.communityId, data.userId);
  if (memberExisting) throw AppErr(409, '用户已经是社区成员');
...
export async function getMemberWithInviter(memberId: string): Promise<CommunityMemberWithInviter> {
  ...
  if (!result) throw AppErr(404, '成员不存在');
